...
declare global {
  ...
  var timetable_entries_cache: Record<string, {
    last_fetch: number,
    entries: OnlineTimetableFileEntry[]
  }>;
}
...
export async function getCachedEntries(year: TimetableYear): Promise<OnlineTimetableFileEntry[]> {
  let entries: OnlineTimetableFileEntry[] | undefined;
  ...
  if (!entries) {
    entries = await OnlineTimetable.getTimetableEntries(year);
...
export async function getEntryAsIs(entry: OnlineTimetableFileEntry): Promise<ITimetable> {
  const base_timetable = await entry.getTimetable();
  const last_update = entry.lastUpdated;
...
export async function getCachedTimetable(entry: OnlineTimetableFileEntry): Promise<ITimetable> {
  let timetable: ITimetable | undefined;
  let shouldUpdateDatabase = false;

  const week_number = entry.weekNumber;
  const db_timetable = await DBTimetable(entry.fromYear).findOne({
  ...
  if (!timetable) {
    ...
    if (shouldUpdateDatabase) {
      ...
      await DBTimetable(entry.fromYear).findOneAndUpdate({
    ...
    // Otherwise, we don't have the timetable inside the DB.
      ...
      await DBTimetable(entry.fromYear).create({
        last_fetch,
        header: timetable.header,
        lessons: timetable.lessons,
        last_update: timetable.last_update
